...
exports.inngestApi = functions
        ...
        try {
            ...
            for (let i = 0; i < prompts.length; i++) {
                ...
                const segmentUrl = await step.run(`generate-segment-${i}`, async () => {
                    ...
                    const requestBody = {
                        instances: [
                            Object.assign({ prompt: prompt }, (currentStartImage ? {
                                image: {
                        ...
                        parameters: {
                            sampleCount: 1,
                            videoLength: (options === null || options === void 0 ? void 0 : options.duration) || "5s",
                            aspectRatio: (options === null || options === void 0 ? void 0 : options.aspectRatio) || "16:9"
                        }
                    };
                    const response = await fetch(endpoint, {
    ...
    const stitchVideoFn = inngestClient.createFunction({ id: "stitch-video-segments" }, { event: "video/stitch.requested" }, async ({ event, step }) => {
        ...
        try {
            ...
            if (!jobName)
                throw new Error("Failed to start transcoder job");
            // Poll for completion
            let attempts = 0;
            const maxAttempts = 60; // 5 minutes (5s * 60)
            while (attempts < maxAttempts) {
                attempts++;
                await step.sleep(`poll-interval-${attempts}`, "5s");
                const status = await step.run(`check-job-status-${attempts}`, async () => {
                ...
                if (status === "SUCCEEDED") {
                    ...
                    await step.run("finalize-success", async () => {
                        ...
                        // For simplicity using storage.googleapis.com format for public buckets
                        // or we could use getSignedUrl if needed.
                        // Assuming typical public access for this use case as per previous code style:
                        const finalUrl = `https://storage.googleapis.com/${bucket.name}/videos/${userId}/${jobId}_final.mp4`;
                        await admin.firestore().collection("videoJobs").doc(jobId).set({
                            status: "completed",
                            videoUrl: finalUrl,
                            progress: 100,
